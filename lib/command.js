...
Command.prototype._ensureArgs = function() {
  if (this._args.length === 0 && this._hasCommands()) {
    this._args.push({ name: 'command', required: true });
  }
};

Command.prototype._hasCommands = function() {
  return Object.keys(this._commands || {}).length > 0;
};

Command.prototype.execute = function(args) {
  var exit = function(defaultCode) {
    return function(result) {
      var n = parseInt(result, 10);
      process.exit(isNaN(n) ? defaultCode : n);
    };
  };
  args = args || process.argv;
  this._basename = path.basename(args[1], path.extname(args[1]));
  return Promise.resolve().then(function() {
    return this._execute(args.slice(2))
  }.bind(this)).then(exit(0), exit(1));
};
...
Command.prototype._execute = function(args) {
  ...
  if (subcommand) {
    ...
    if (binded.missings.length > 0) {
      ...
      return this._action.apply(this, binded.parsed.concat([options]));
    }
  }
};
...
Command.prototype.usageHelp = function() {
  var args = (this._args.length > 0 ? this._args.map(function(i) {
    return (i.required ? '<' : '[') + i.name + (i.required ? '>' : ']');
  }).join(' ') : '');
  return '  Usage: ' + (this._name || this._basename) + (args ? ' ' + args : '');
};

Command.prototype.commandHelp = function() {
  var width = Object.keys(this._commands).reduce(function(w, name) {
    return Math.max(w, name.length)
  }, 0);
  ...
  return !this._hasCommands() ? '' : [
    ...
    Object.keys(this._commands).map(function(name) {
      ...
      return '    ' + rpad(name, width) + ' ' + (desc ? desc : '');
    }, this).join('\n'),
  ].join('\n');
};
...
Command.prototype.exampleHelp = function() {
  ...
  return [
    ...
    '  Examples: ',
    '',
    this._examples.map(function(s) { return '    ' + s; }).join('\n')
  ].join('\n');
};
...
module.exports._parseExpectedArgs = parseExpectedArgs;
